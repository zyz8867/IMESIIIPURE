...
/**
 * Created By YPT on 2019/4/1/001
...
public class MapUtils {
    ...
    public static void x_openGDMap(Context context, String lat, String lon) {
        Intent intent = new Intent();
        intent.setAction("android.intent.action.VIEW");
        intent.setData(Uri.parse("androidamap://viewMap?sourceApplication=appname" + "&lat=" + lat + "&lon=" + lon + "&dev=0"));
     ...
     * @param lon
     */
    public static void x_openBdMap(Context context, String lat, String lon) {


        Intent intent = new Intent();

        intent.setAction("android.intent.action.VIEW");
        intent.setData(Uri.parse("baidumap://map/marker?location="+lat+","+lon+"&title=事发地点&content=请立即前往&traffic=on&src=diandian"));

        if (isBaiduMapInstalled()) {

            context.startActivity(intent);
        } else {
            Toast.makeText(context, "请先安装百度地图app", Toast.LENGTH_SHORT).show();
        }
    }
